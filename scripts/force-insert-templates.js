/**
 * Create nr_templates table using Supabase Management API
 * This bypasses PostgREST and uses database connection directly
 */
...
console.log('🚀 Criando tabela nr_templates via múltiplas inserções...\n');

// Dados dos 10 templates
const templates = [
  {
    nr_number: 'NR-01',
...
console.log('⚠️  ATENÇÃO: A tabela precisa ser criada manualmente primeiro!\n');
console.log('📋 Instruções:');
console.log('1. Acesse: https://supabase.com/dashboard/project/ofhzrdiadxigrvmrhaiz/sql');
console.log('2. Execute o SQL de: supabase/migrations/20251118000000_create_nr_templates_table.sql\n');
console.log('Após criar a tabela, este script inserirá os 10 templates automaticamente.\n');

// Aguardar 5 segundos para o usuário ler
await new Promise(resolve => setTimeout(resolve, 3000));

console.log('🔍 Tentando inserir templates (vai falhar se tabela não existir)...\n');
...
for (const template of templates) {
  try {
    ...
    if (error) {
      errors.push({ template: template.nr_number, error: error.message });
      console.log(`❌ ${template.nr_number}: ${error.message}`);
    } else {
      successCount++;
      console.log(`✅ ${template.nr_number}: ${template.title.substring(0, 50)}...`);
    }
  } catch (err) {
    errors.push({ template: template.nr_number, error: err.message });
    console.log(`❌ ${template.nr_number}: ${err.message}`);
  }
}

console.log(`\n═══════════════════════════════════════════════════`);
if (successCount === 10) {
  console.log(`✅ SUCESSO! ${successCount}/10 templates inseridos`);
  console.log(`\n🎯 Fase 9: 100% COMPLETA!`);
} else if (successCount > 0) {
  console.log(`⚠️  Parcial: ${successCount}/10 templates inseridos`);
  console.log(`\nErros (${errors.length}):`);
  errors.forEach(e => console.log(`  - ${e.template}: ${e.error}`));
} else {
  console.log(`❌ FALHOU: Tabela não existe`);
  console.log(`\n📋 Execute manualmente:`);
  console.log(`   https://supabase.com/dashboard/project/ofhzrdiadxigrvmrhaiz/sql`);
  console.log(`   Arquivo: supabase/migrations/20251118000000_create_nr_templates_table.sql`);
}
